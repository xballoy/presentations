...
describe('Sellers', function () {
  ...
  it('should return all sellers sorted by cash in decreasing order', function () {
    sellers.save(bob)
    const alice = { name: 'alice', hostname: 'hostname', port: '3001', path: '/path', cash: 10.0 }
    sellers.save(alice)
    const carol = { name: 'carol', hostname: 'hostname', port: '3002', path: '/path', cash: 5.0 }
    sellers.save(carol)
  ...
  it('should update sellers data and preserve cash & cash history', function () {
    ...
    const newBob = { name: bob.name, hostname: 'new hostname' }
    sellers.save(newBob)
...
describe('Countries', function () {
  ...
  it('should get the updated tax for a given country', function () {
    spyOn(configuration, 'all').andReturn({})
    const newTaxRule = function (total) { if (total > 100) return total * 1.2; else return total * 1.2 + 100 }
