...
const SellerCashUpdater = services.SellerCashUpdater
const BadRequest = services.BadRequest
const OrderService = services.OrderService
const SellerService = services.SellerService
const Reduction = services.Reduction
const Countries = repositories.Countries
const Sellers = repositories.Sellers
...
describe('Seller Service', function () {
  let sellers, sellerService, bob, configurationData
  ...
  it('should deduct a penalty when a seller is offline', function () {
    const bob = { name: 'bob', cash: 200, online: true }
  ...
  it('should send notification to seller', function () {
    spyOn(utils, 'post')
    const message = { type: 'info', content: 'test' }
  ...
  it('should authorized seller if the same username and password are provided', function () {
    const travis = { name: 'travis', password: 'pacman' }
    sellers.save(travis)
...
describe('Order Service', function () {
  let orderService, configuration
  let countries
  ...
  it('should create an order with maximum 10 items', function () {
    const order = orderService.createOrder(Reduction.STANDARD)
  ...
  it('should calculate the sum of the order using PAY_THE_PRICE reduction', function () {
    ...
    const order = { prices: [1000, 50], quantities: [1, 2], country: 'IT' }

    const bill = orderService.bill(order, Reduction.PAY_THE_PRICE)
  ...
  it('should calculate the sum of the order using STANDARD reduction', function () {
    ...
    const bill = orderService.bill(order, Reduction.STANDARD)
  ...
  it('should calculate the sum of the order using HALF_PRICE reduction', function () {
    ...
    const bill = orderService.bill(order, Reduction.HALF_PRICE)
...
describe('Dispatcher', function () {
  let dispatcher, orderService, sellerService, configuration
  ...
  it('should send the same order to each seller using reduction', function () {
    spyOn(configuration, 'all').andReturn({})
    const alice = { name: 'alice', hostname: 'seller', port: '8080', path: '/', cash: 0 }
    const bob = { name: 'bob', hostname: 'seller', port: '8081', path: '/', cash: 0 }
    spyOn(sellerService, 'addCash')
    spyOn(sellerService, 'allSellers').andReturn([alice, bob])
    const order = { prices: [100, 50], quantities: [1, 2], country: 'IT' }
...
describe('Seller\'s cash updater', function () {
  let sellerCashUpdater, configuration, sellerService, orderService
...
describe('BadRequest', function () {
  let badRequest, sellerService, sellers, configuration
  ...
  it('should deduct cash if response status is not "bad request"', function () {
    const self = { sellerService }
    const seller = { name: 'alice', cash: 200 }
    const expectedBill = { total: 47 }
    const currentIteration = 17
    spyOn(sellerService, 'deductCash')
    spyOn(sellerService, 'notify')

    const fun = badRequest.updateSellersCash(self, seller, expectedBill, currentIteration)
